...
def analyze_fixture(db: Session, fixture: Fixture) -> FixtureAnalysis:
    ...
    criteria = [
        _make_criterion(
            "combined_avg_goals",
            combined_avg,
            f">= {comp_profile.min_combined_avg_goals} gols/jogo",
            combined_avg >= comp_profile.min_combined_avg_goals,
            "Média combinada de gols marcados e sofridos",
            higher_is_better=True,
            strong_at=comp_profile.strong_combined_avg_goals,
        ),
        _make_criterion(
            "max_zero_zero_rate",
            max_zero_zero,
            f"<= {comp_profile.max_zero_zero_rate} ({comp_profile.max_zero_zero_rate:.0%})",
            max_zero_zero <= comp_profile.max_zero_zero_rate,
            "Pior taxa de 0-0 entre as duas seleções",
            higher_is_better=False,
            strong_at=comp_profile.strong_max_zero_zero_rate,
        ),
        _make_criterion(
            "min_over_05_rate",
            min_over_05,
            f">= {comp_profile.min_over_05_historical_rate} ({comp_profile.min_over_05_historical_rate:.0%})",
            min_over_05 >= comp_profile.min_over_05_historical_rate,
            "Menor taxa histórica de over 0,5",
            higher_is_better=True,
            strong_at=comp_profile.strong_over_05_historical_rate,
        ),
        _make_criterion(
            "both_teams_score_rate",
            avg_btts,
            f">= {comp_profile.min_both_score_rate} ({comp_profile.min_both_score_rate:.0%})",
            avg_btts >= comp_profile.min_both_score_rate,
            "Média de jogos em que ambas marcam",
            higher_is_better=True,
            strong_at=comp_profile.strong_both_score_rate,
        ),
        _make_criterion(
            "home_offense",
            home_profile.avg_goals_scored,
            f">= {comp_profile.min_offense_goals} gols/jogo",
            home_profile.avg_goals_scored >= comp_profile.min_offense_goals,
            f"{localize_team_name(home.name, home.external_id)} — gols marcados/jogo",
            higher_is_better=True,
            strong_at=comp_profile.strong_offense_goals,
        ),
        _make_criterion(
            "away_offense",
            away_profile.avg_goals_scored,
            f">= {comp_profile.min_offense_goals} gols/jogo",
            away_profile.avg_goals_scored >= comp_profile.min_offense_goals,
